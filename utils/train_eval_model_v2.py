...
def evaluate_loss_with_threshold_reconstruct(net, data_iter, mask_percentage, q_list, device=None):
    if isinstance(net, nn.Module):
        net.eval()
        if not device:
            device = next(iter(net.parameters())).device
...
def train_reconstruct(net, train_iter, test_iter, num_epochs, learning_rate, mask_percentage, q_list,
                      patience, devices, logger, weights_save_parent_path):
    ...
    for epoch in range(num_epochs):
        ...
        test_loss, threshold_list = evaluate_loss_with_threshold_reconstruct(net, test_iter, mask_percentage, None)
...
def evaluate_acc_loss_classify(net, data_iter, loss, device=None):
    if isinstance(net, nn.Module):
        net.eval()  # 设置为评估模式
        if not device:
            device = next(iter(net.parameters())).device
...
def train_classify(net, train_iter, test_iter, num_epochs, learning_rate, patience,
                   devices, logger, weights_save_parent_path):
    animator = None if len(devices) > 1 else d2l.Animator(xlabel='epoch', xlim=[1, num_epochs],
    ...
    for epoch in range(num_epochs):
        ...
        logger.record_logs([
                               f'epoch: {epoch + 1}, current patience: {current_patience + 1}, test loss: {test_loss:.3f}, test acc: {test_acc:.3f}'])
        weights_save_path = os.path.join(weights_save_parent_path, f"epoch_{epoch + 1}.pth")
        if (epoch + 1) % 5 == 0 or (epoch + 1) == num_epochs:
            torch.save(net.state_dict(), weights_save_path)
        if test_loss < best_test_loss:
